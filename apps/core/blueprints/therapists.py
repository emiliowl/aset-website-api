...
def get_all():
    try:
        ...
        therapists_json = json.dumps([t.to_dict() for t in therapists], default=default)
        return Response(therapists_json, mimetype='application/json', status=200)
...
def get(email):
    try:
        ...
        return Response(therapist_json, mimetype='application/json', status=200)
    except Exception as ex:
        print(ex)
        abort(500)
